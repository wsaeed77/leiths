...
jQuery( function( $ ) {
	...
	TokenizationForm.prototype.hideForm = function() {
		console.log( "hide" );
		$( '.not-for-token', this.$formWrap ).hide();
		$( "#sage-card-cvc" ).removeClass( "form-row-last" );
		$( "#sage-card-cvc" ).addClass( "form-row-wide" );

		// Remove Opayo Pi Dropin form
		// $("#woocommerce-sp-container").empty();
	};

	TokenizationForm.prototype.showForm = function() {
		console.log( "show" );
		$( '.not-for-token', this.$formWrap ).show();
		$( "#sage-card-cvc" ).removeClass( "form-row-wide" );
		$( "#sage-card-cvc" ).addClass( "form-row-last" );
	};

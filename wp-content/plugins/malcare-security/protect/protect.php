...
if (!class_exists('MCProtect_V565')) :
...
class MCProtect_V565 {
	...
	public static function init($mode) {
		...
		if ($mode == MCProtect_V565::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = MCProtectUtils_V565::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(MCProtect_V565::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new MCProtectRequest_V565($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			MCProtectFW_V565::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			MCProtectFW_V565::getInstance($mode, $request, $fw_config, $brand_name)->init();
			MCProtectLP_V565::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		MCProtectIpstore_V565::uninstall();
		MCProtectFW_V565::uninstall();
		MCProtectLP_V565::uninstall();

		MCProtect_V565::removeWPPrepend();
		MCProtect_V565::removePHPPrepend();
		MCProtect_V565::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		MCProtectUtils_V565::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		MCProtect_V565::removeHtaccessPrepend();
		MCProtect_V565::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		MCProtectUtils_V565::fileRemovePattern(dirname(ABSPATH) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		MCProtectUtils_V565::fileRemovePattern(dirname(ABSPATH) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		MCProtectUtils_V565::rrmdir($mc_data_dir);
	}
}
endif;
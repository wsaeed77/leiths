...
abstract class Daq_Form_Element_Multi extends Daq_Form_Element 
{
     ...
     * @var string
     */
    protected $_contentDisplay = "inline";
    ...
    /**
     * Sets content display
     * 
     * @param string $method    One of "list", "inline"
     * @throws Exception        If unallowed content dispaly set
     */
    public function setContentDisplay( $cd ) 
    {
        if( !in_array( $cd, array("list", "inline" ) ) ) {
            throw new Exception("Unknown content display method [$cd].");
        }

        $this->_contentDisplay = $cd;
    }
    ...
    public function overload(array $data) 
    {
        ...
        if(isset($data["content_display"]) && $data["content_display"] > 0) {
    ...
    public function dump() 
    {
        ...
        if( isset( $this->_overload["fill_method"] ) ) {
            $dump->fill_method = $this->_overload["fill_method"];
        }
        if( isset( $this->_overload["fill_choices"] ) ) {
            $dump->fill_choices = $this->_overload["fill_choices"];
        }
        if( isset( $this->_overload["fill_callback"] ) ) {

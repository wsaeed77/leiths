...
abstract class Daq_Db_OrmAbstract
{
    ...
    public function __construct($id = null, $prefix = true)
    {
        ...
        foreach(Daq_Db::getInstance()->describeTable($this->_name) as $i => $row) {
    ...
    public function save()
    {
        ...
        foreach($this->_field as $key => $value) {
            ...
            switch($value["type"]) {
                ...
                //case "text": $value["value"] = htmlspecialchars( strip_tags( strval($value["value"] ) ) ); break;
                //case "textarea": $value["value"] = $this->wpjb_strip_tags( strval($value["value"] ) ); break;
            }
    ...
    protected function wpjb_strip_tags( $text ) {

        if( is_numeric( $text ) || is_date( $text ) ) {
            return $text;
        }

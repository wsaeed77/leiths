...
class Daq_Tpl_Email {
     ...
     * @var array
     */
    public $path_replace_list = array('a', 'img', 'link', 'script', 'input');
    ...
    public function draw($tpl) {
        ...
        try {
            ...
            $tpl = $this->compileTemplate($tpl);
        } catch (Daq_Tpl_Exception $e) {
     ...
     * @access protected
     */
    protected function compileFile($template_code, $template_basedir) {
        //xml substitution
        ...
        //compile template
        $template_compiled = $this->compileTemplate($template_code, $template_basedir);
     ...
     * @access protected
     */
    protected function compileTemplate($template_code, $tpl_basedir = null) {
        ...
        //compile the code
        $compiled_code = $this->_compileCode($template_code);
     ...
     * @access protected
     */
    protected function _compileCode($parsed_code) {
        ...
        if ($open_if > 0) {
            $e = new Daq_Tpl_SyntaxException('Error! You need to close an {if} tag in ' . $this->tpl['tpl_filename'] . ' template');
     ...
     *
     * @param Daq_Tpl_Exception $e
     * @return string
     */
    protected function printDebug(Daq_Tpl_Exception $e) {
        if (!$this->debug) {
            throw $e;
        }
        $output = sprintf('<h2>Exception: %s</h2><h3>%s</h3><p>template: %s</p>', get_class($e), $e->getMessage(), $e->getTemplateFile()
        );
        if ($e instanceof Daq_Tpl_SyntaxException) {

...
<div class="wpjb wpjb-page-company-products">
    ...
    <?php //if( strtotime( $summary->started_at ) > time() ): continue; endif; ?>
    <?php $has_active = true; ?>
    ...
    <div class="wpjb-company-product">
        ...
        <div class="wpjb-company-product-subheader">
            <div class="wpjb-company-product-indicator">
                <?php if( strtotime( $summary->started_at ) > time() ): ?>
                    <span class="wpjb-glyphs wpjb-icon-calendar"></span>
                    <?php printf( __('Days to start: %1$d'), ( strtotime( $summary->started_at ) - strtotime(date("Y-m-d") ) ) / ( 24*3600 ) ); ?>
                <?php endif; ?>
                <span class="wpjb-glyphs wpjb-icon-calendar"></span>
        ...
        <div class="wpjb-company-product-details">
            ...
            <?php $single_job = $summary->bundle[Wpjb_Model_Pricing::PRICE_SINGLE_JOB]; ?>
            <?php $single_resume = $summary->bundle[Wpjb_Model_Pricing::PRICE_SINGLE_RESUME]; ?>

            <?php if(!empty($single_job)): ?>
            <?php foreach($single_job as $usage): ?>
                <?php $id = $usage["id"]; ?>
                ...
                <div class="wpjb-company-product-details-single">
                    ...
                    <?php else: ?>
                        <?php printf( __('Uses: %1$d / %2$d', "wpjobboard"), $usage["used"], $usage["usage"] ); ?>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
            <?php endif; ?>
            ...
            <?php foreach($single_resume as $usage): ?>
                <?php $id = $usage["id"]; ?>
                <?php $product = new Wpjb_Model_Pricing($id); ?>
                <div class="wpjb-company-product-details-single">
                    <strong><?php esc_html_e( $product->title ) ?></strong>
            ...
            <?php if( isset( $pricing->meta->have_access ) ): ?>
            <div class="wpjb-company-product-details-single">
                <?php _e("<strong>Have access to pages:</strong>", "wpjobboard" ); ?> <br/>
                <?php foreach( explode( ",", $pricing->meta->have_access->value() ) as $page_id ): ?>
        ...
        <div class="wpjb-company-product-actions">
            <div class="wpjb-company-product-actions-left">
                ...
                <a href="<?php echo get_permalink( wpjb_conf('urls_link_membership_pricing') ) . sprintf("?membership_id=%d", $pricing->id) ?>" class="wpjb-manage-action wpjb-glyphs wpjb-icon-arrows-cw"><?php _e("Rebuy Package", "wpjobboard"); ?></a>
                <?php endif; ?>

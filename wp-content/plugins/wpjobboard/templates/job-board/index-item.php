...
?>
    ...
    <div class="wpjb-grid-row wpjb-grid-flex wpjb-click-area <?php wpjb_job_features($job); ?>">
        <?php if($image != "none"): ?>
        <div class="wpjb-grid-col wpjb-col-logo">
            <?php if($job->doScheme("company_logo")): ?>
            <?php elseif($job->getLogoUrl()): ?>
            <div class="<?php echo apply_filters( "wpjb_tpl_list_col_img", "wpjb-flex-img wpjb-flex-img-job", $job ) ?>">
                <img src="<?php echo $job->getLogoUrl($image) ?>" alt="" class="" />
            </div>
            <?php elseif($job->getCompany(true)->getLogoUrl()): ?>
            <div class="<?php echo apply_filters( "wpjb_tpl_list_col_img", "wpjb-flex-img wpjb-flex-img-company", $job ) ?>">
                <img src="<?php echo $job->getCompany(true)->getLogoUrl($image) ?>" alt="" class="" />
            </div>
            <?php elseif(isset($image_default_url) && $image_default_url): ?>
            <div class="<?php echo apply_filters( "wpjb_tpl_list_col_img", "wpjb-flex-img wpjb-flex-img-default", $job ) ?>">
                <img src="<?php echo esc_attr($image_default_url) ?>" alt="" class="" />
            </div>
            <?php else: ?>
            <div class="<?php echo apply_filters( "wpjb_tpl_list_col_img", "wpjb-flex-img wpjb-flex-img-bg", $job ) ?>">

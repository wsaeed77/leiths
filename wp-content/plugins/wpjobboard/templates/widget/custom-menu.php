    ...
    <?php foreach($param->structure as $key => $link): ?>
        ...
        <?php else: ?>
            <li class="wpjb-custom-menu-link <?php esc_attr_e($link["menu-item-classes"]) ?>">
                ...
                <?php endif; ?>
                <a href="<?php esc_html_e(wpjb_custom_menu_key_to_url($link["menu-item-key"])) ?>"><?php esc_html_e($link["menu-item-title"]) ?></a>
            </li>
        <?php endif; ?>
            
    <?php endforeach; ?>
    <?php else: ?>
    <li><?php _e("No links in the menu", "wpjobboard") ?></li>
    <?php endif; ?>
</ul>

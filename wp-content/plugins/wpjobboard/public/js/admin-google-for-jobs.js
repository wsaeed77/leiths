...
WPJB.GFJ.Map.Item.MonetaryAmount.prototype.getFormData = function() {
    ...
    var data = {
        ...
        path: {
            ...
            minValue: {
                value: this.value.minValue.name
            },
            maxValue: {
                value: this.value.maxValue.name
            },
            unitText: {
                value: this.value.unitText.name
            }
        }
    };
    ...
    if(data.path.minValue.value == "text") {
        data.path.minValue.text = this.value.minValue.text;
    }

    if(data.path.maxValue.value == "text") {
        data.path.maxValue.text = this.value.maxValue.text;
    }

    if(data.path.unitText.value == "text") {
        data.path.unitText.text = this.value.unitText.text;
    }
...
WPJB.GFJ.Map.Item.MonetaryAmount.prototype.save = function(item) {
    ...
    var value = item.find(".wpjb-gfj-item-edit-input[data-name='value']");
    var minvalue = item.find(".wpjb-gfj-item-edit-input[data-name='minValue']");
    var maxvalue = item.find(".wpjb-gfj-item-edit-input[data-name='maxValue']");
    var currency = item.find(".wpjb-gfj-item-edit-input[data-name='currency']");
    var unittext = item.find(".wpjb-gfj-item-edit-input[data-name='unitText']");
    ...
    var data = {
        ...
        minValue: {
            label: minvalue.find("option:selected").text(),
            name: minvalue.find("option:selected").val()
        },
        maxValue: {
            label: maxvalue.find("option:selected").text(),
            name: maxvalue.find("option:selected").val()
        },
        unitText: {
            label: unittext.find("option:selected").text(),
            name: unittext.find("option:selected").val()
        }
    };
    ...
    if(data.minValue.name == "text") {
        data.minValue.text = minvalue.find(".wpjb-gfj-internal-text-value").val();
    }

    if(data.maxValue.name == "text") {
        data.maxValue.text = maxvalue.find(".wpjb-gfj-internal-text-value").val();
    }

    if(data.unitText.name == "text") {
        data.unitText.text = unittext.find(".wpjb-gfj-internal-text-value").val();
    }
...
jQuery(function($) {
    ...
    $.each(WPJB_GFJ_DATA.MAP, function(index, item) {
        ...
        if(jQuery.inArray(type, ["Text", "Date", "Url"]) >= 0) {
            ...
            if( typeof item.path.minValue === 'undefined' ) {
                item.path.minValue = { value: null };
            }
            if( typeof item.path.maxValue === 'undefined' ) {
                item.path.maxValue = { value: null };
            }
            if( typeof item.path.unitText === 'undefined' ) {
                item.path.unitText = { value: null };
            }
            ...
            data.value = {
                ...
                minValue: {
                    name: item.path.minValue.value,
                    label: WPJB.GFJ.getLabelFor(item.path.minValue.value),
                    text: WPJB.GFJ.getTextFrom(item.path.minValue)
                },
                maxValue: {
                    name: item.path.maxValue.value,
                    label: WPJB.GFJ.getLabelFor(item.path.maxValue.value),
                    text: WPJB.GFJ.getTextFrom(item.path.maxValue)
                },
                unitText: {
                    name: item.path.unitText.value,
                    label: WPJB.GFJ.getLabelFor(item.path.unitText.value),
                    text: WPJB.GFJ.getTextFrom(item.path.unitText)
                }

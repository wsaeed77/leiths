jQuery(function($) {
    ...
    jQuery("#job_created_at").datepicker({
        dateFormat: wpjb_admin_job_lang.datepicker_date_format,
        //autoSize: true,
        changeMonth: false,
        changeYear: true,
        yearRange: "c-100:c+5",
        onSelect: function( date ){
            jQuery("#job_created_at").attr("value", date);
            wpjb_job_dates();
        }
    });
    
    $("#job_created_at_link").click(function( e ) {
        $("#job_created_at").focus().focus();
        return false;
    });
    ...
    jQuery("#job_expires_at").datepicker({
        ...
        changeYear: true,
        onSelect: function( date ){
            jQuery("#job_expires_at").attr("value", date);
            jQuery(".job-expires-at-formated").attr("title", date);
            wpjb_job_dates();
        }
    });
    ...
    $("#job_expires_at_link").click(function() {
        ...
        if( $("#job_expires_at").val() == wpjb_admin_job_lang.max_date ) {
            $("#job_expires_at").val( today );
        }

        $("#job_expires_at").focus().focus();
        return false;
    });
...
function wpjb_job_dates() {
    ...
    var d = wpjb_date_diff(new Date(), new Date(created));
    var t = created;

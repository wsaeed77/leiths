...
class Wpjb_Controller_Admin extends Daq_Controller_Abstract
{
    ...
    public function getNonceName($action = null, $page = null) {
        if( $page === null ) {
            $page = $this->_request->get("page", "");
        }

        if( $action === null ) {
            $action = $this->_request->get("action", "default");
        }

        return sprintf( "%s-%s", $page, $action);
    }
    ...
    {   
        ...
        $id = false;
        $this->view->nonce = $this->getNonceName();
    ...
    public function editAction()
    {
        ...
        $f = new $form($id);
        $f->addElement($this->_getNonceField(), "hidden");
        $f->addElement($this->_getRefField(), "hidden");
    ...
    protected function _getNonceField() {
        $e = new Daq_Form_Element_Hidden("_wpjb_nonce");
        $e->setValue(wp_create_nonce($this->getNonceName()));

        
        return $e;
    }

    public function _getRefField() {
        $request_url = remove_query_arg( '_wp_http_referer' );

        $e = new Daq_Form_Element_Hidden("_wp_http_referer");
        $e->setValue($request_url);
    ...
    {
        if( ! wp_verify_nonce( $this->_request->post("_wpjb_nonce"), $this->getNonceName("index") ) ) {
            wp_die(__("Invalid Nonce.", "wpjobboard"));
        }
    ...
    public function deriveParams($query, $object = null)
    {
        ...
        $replace = str_replace(array("[", "]"), " ", (string)$query);

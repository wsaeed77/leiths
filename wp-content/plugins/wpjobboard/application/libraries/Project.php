...
class Wpjb_Project extends Daq_ProjectAbstract
{
    ...
    public function runAdmin() {
        ...
        add_filter('set_screen_option_wpjb_screen', array($so, "setScreenOptions"), 11, 3);
    ...
    public function setupSchedule() {
        ...
        if ( ! wp_next_scheduled( 'wpjb_event_expiring_jobs' ) ) {
            wp_schedule_event( time(), 'daily', 'wpjb_event_expiring_jobs');
        }

        if ( ! wp_next_scheduled( 'wpjb_event_subscriptions_daily' ) ) {
            wp_schedule_event( time(), 'hourly', 'wpjb_event_subscriptions_daily');
        }

        if ( ! wp_next_scheduled( 'wpjb_event_subscriptions_weekly' ) ) {
            wp_schedule_event( time(), 'hourly', 'wpjb_event_subscriptions_weekly');
        }

        if ( ! wp_next_scheduled( 'wpjb_event_clean_memberships' ) ) {
            wp_schedule_event( time(), 'daily', 'wpjb_event_clean_memberships');
        }
    ...
    public function run()
    {
        ...
        if(wpjb_conf("fs_enabled")) {
            add_filter( "wpjb_upload_dir", "wpjb_upload_dir_secure", 10, 5);
        }
    ...
    public static function scheduleEvent()
    {
        ...
        if( apply_filters( "wpjb_skip_jobs_count", false ) ) {
            return;
        }
    ...
    public function deletedUser($id)
    {
        ...
        if( $pagenow != "users.php" || Daq_Request::getInstance()->post( "action" ) != "dodelete" ) {
            return;
        }
    ...
    public function recaptcha($form)
    {
        ...
        $e->setAttr( "__class", get_class( $form ) );

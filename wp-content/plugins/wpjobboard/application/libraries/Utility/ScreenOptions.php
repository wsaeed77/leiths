...
class Wpjb_Utility_ScreenOptions 
{
    ...
    public function dataApplication()
    {
        return array(
            "fields" => array(
                ...
                "status" => array("label"=>__("Status", "wpjobboard"), "checked"=>0),
            ),
            "columns" => array(
                "id" => 0, 
                "__gravatar" => 1,
    ...
    public function doScreenApplication() {
        ...
        add_action("wpjb_custom_screen_filters", array( $this, "customFilters"), 10, 1);   
        
        $filters = $this->_findFilters($key, $data);
    ...
    public function dataJob() {
        return array(
            "fields" => array(
                ...
                "job_address" => array( "label" => __("Address", "wpjobboard"), "checked" => 0 ),
    ...
    protected function _findFilters($key, $data ) {
        ...
        if(!isset($columns[$key]["filters"])) {
            $columns = $data["filters"];
        } else {
            $columns = array_combine($columns[$key]["filters"], array_fill(0, count($columns[$key]["filters"]), 1));
        }
        ...
        self::$_filters[$key] = $this->_findFields( $data, $columns );
        
        return self::$_filters[$key];
    }
    
        
    protected function _findFields( $data, $columns, $exclude = null ) {
        $form = new $data["form"];
        $fields = array();
        ...
        foreach($groups as $group) {
            foreach($group->getReordered() as $field) {
                ...
                $exclude = array("hidden", "password", "file", "label");
    ...
    protected function _renderHtml($key, $fields) {
        ...
        $screen .= $this->_renderFilters($key);
    ...
    protected function _renderFilters($key) {
        if(!isset(self::$_filters[$key]) || empty(self::$_filters[$key])) {
            return;
        }
        
        $screen = "";
        $screen .= sprintf("<h5>%s</h5>", __("Additional Filters", "wpjobboard"));
        $screen .= '<div class="wpjb-screen-fields">';

        foreach(self::$_filters[$key] as $name => $f) {
            $input = new Daq_Helper_Html("input", array(
                "id" => "wpjb_filter_".$name,
                "type" => "checkbox",
                "name" => "wpjb_screen[$key][filters][]",
                "value" => $name,
                "checked" => $f["checked"]
            ));

            $label = new Daq_Helper_Html("label", array("for"=>"wpjb_filter_".$name), $input->render().' '.$f["label"]);
    ...
    public function customFilters($key, $objct = null) {
        $uscr = get_user_option("wpjb_screen"); 
        $frm = new Wpjb_Form_Apply();
        $object = $frm->getObject();
        if(!isset($uscr[$key]) || !isset($uscr[$key]["filters"])) {
            return;
        }

        foreach($uscr["application"]["filters"] as $filter) {
            if(!$frm->hasElement($filter)) {
                continue;
            }

            $e = $frm->getElement($filter);

            $toText = array("text", "textarea");
            $toDropdown = array("select", "checkbox", "radio");

            if(in_array($e->getType(), $toText)) {
                $input = $this->_inputToText($e);
            } elseif(in_array($e->getType(), $toDropdown)) {
                $input = $this->_inputToDropdown($e);
            } else {
                continue;
            }
            ...
            ?>
            <div>
                <label for=""><?php echo esc_html($e->getLabel()) ?></label>
                <?php echo $input ?>
            </div>
            <?php
    ...
    protected function _inputToText($e) {
        $search = Daq_Request::getInstance()->get("search");
        if(isset($search["meta"]) && isset($search["meta"][$e->getName()])) {
            $value = $search["meta"][$e->getName()];
        } else {
            $value = "";
        }

        
        $input = new Daq_Form_Element_Text(sprintf( "search[meta][%s]", $e->getName()));
        $input->setValue($value);
        return $input;
    }

    protected function _inputToDropdown($e) {
        $search = Daq_Request::getInstance()->get("search");
        if(isset($search["meta"]) && isset($search["meta"][$e->getName()])) {
            $value = $search["meta"][$e->getName()];
        } else {
        ...
        $input = new Daq_Form_Element_Select(sprintf( "search[meta][%s]", $e->getName()));
        $input->setEmptyOption(true);
        foreach($e->getOptions() as $o) {
            //print_r($option);
            $input->addOption($o["key"], $o["value"], $o["desc"], $o["param"]);
        }
        $input->setValue($value);

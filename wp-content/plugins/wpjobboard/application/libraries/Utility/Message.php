...
class Wpjb_Utility_Message
{
    ...
    public function assign($var, $value)
    {
        ...
        $files_to_include = json_decode( $this->_template->files, true );
        
        if( !$files_to_include && ( $this->_template->name == "notify_employer_new_application" || $this->_template->name == "notify_admin_new_application" ) ) {
            $files_to_include["mail_file_application"][] = "file";
        }
        
        $file_name = "mail_file_" . $var;
        if( isset( $files_to_include[$file_name] ) ) {
            
            $files = array();
            foreach( $files_to_include[$file_name] as $field_name ) {
                
                $upload = wpjb_upload_dir( $var , str_replace( "_", "-", $field_name ), $value['id'] );
                $upload = $upload["basedir"]."/*";
                $obj_files = wpjb_glob($upload);     
    ...
    public function addFiles($files) 
    {
        ...
        $this->_file = array_unique( array_merge( $this->_file, $files ) );
    }

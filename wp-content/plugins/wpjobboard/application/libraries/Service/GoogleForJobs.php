...
class Wpjb_Service_GoogleForJobs {
    ...
    public static function getFields() {
        $data = array(
            ...
            "applicantLocationRequirements" => array(
                ...
                "label" => __("Job Location Requirements", "wpjobboard"),
                "is_required" => false,
                "is_recommended" => false
            ),
    ...
    public function getJson($job) {
        ...
        $json->jobLocation->address->streetAddress = $job->job_address;
        ...
        $json->jobLocation->address->addressCountry = $country;
    ...
    protected function _map($json, Wpjb_Model_Job $job, $map) {
        ...
        $special = array(
            ...
            "applicantLocationRequirements" => "_mapApplicantLocationRequirements",
    ...
    /**
     * Map Applicant Location Requirements Object
     * 
     * @param array             $item       Mapping data
     * @param Wpjb_Model_Job    $job        Job Object
     * @param stdClass          $default    Current JSON-LD value
     * @return stdClass                     Customized JSON-LD node
     */
    protected function _mapApplicantLocationRequirements($item, $job, $default) {
        $object = new stdClass();
        $object->{"@type"} = "Country";

        $keys = array("name");
        foreach($keys as $key) {
            //print_r($item);print_r($job);print_r($default);print_r($key);
            //$value = $this->_mapObject($item, $job, $default, $key);
            $value = $this->_mapScalar( $item, $job, $default );
            if(!empty($value)) {
                $object->$key = $value;
            }
        }
    ...
    protected function _mapSalary($item, $job, $default) {
        ...
        $object->value->{"@type"} = "QuantitativeValue";

        $keys = array("value", "currency", "minValue", "maxValue", "unitText");
        foreach($keys as $key) {
            $value = $this->_mapObject($item, $job, $default, $key);

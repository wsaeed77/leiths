...
{
    protected $_custom = "wpjb_form_resume-search";
    
    public function __construct( $id = null, $options = array() )
    {
        if( isset( $options['custom'] ) ) {
            if( $options['custom'] == "wpjb_form_resume-search") {
                $this->_custom = "wpjb_form_resume_search";
            } elseif( strpos( $options['custom'], "wpjb_form_resume-search" ) !== 0 ) {
                $this->_custom = "wpjb_form_resume_search_" . $options['custom'];
            } else {
            ...
            if( isset( $forms_list['resume_search'] ) ) {
                foreach( $forms_list['resume_search'] as $form_code ) {
                    ...
                    if( /*$form_detail['config']['form_is_default'] && */ $form_detail['config']['form_is_active'] ) {
                        $this->_custom = "wpjb_form_resume_search_" . $form_detail['config']['form_code'];
                        break;
                    }
                }
            }

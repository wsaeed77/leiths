...
class Wpjb_Form_AddJob extends Wpjb_Form_Abstract_Job
{
    ...
    public function init()
    {
        ...
        if( wpjb_conf( "allow_set_expiration_date", false ) ) {

            wp_enqueue_script("wpjb-vendor-datepicker");
            wp_enqueue_style("wpjb-vendor-datepicker");

            $duration = apply_filters( "wpjb_default_expiration_duration", wpjb_conf("default_job_duration", 30) );
            $dd = date(wpjb_date_format(), strtotime("today +" . $duration . " day"));

            $e = $this->create("custom_expiration_date", "text_date");
    ...
    public function buildModel( $form_code = null )
    {
        $form_scheme = false;
        $form_scheme_s = get_option( sprintf( "wpjb_form_job_%s", $form_code ) );
        ...
        $duration = absint(wpjb_conf("default_job_duration", 0));
        if($this->value("listing")) {
            ...
            if($listing->exists() && is_object($listing->meta->visible)) {
                $duration = $listing->meta->visible->value();
            }
        }

        if($this->hasElement("custom_expiration_date")) {
            $object->job_expires_at = $this->getElement("custom_expiration_date")->getValue();
        } elseif($duration > 0) {
            $object->job_expires_at = date( "Y-m-d H:i:s", strtotime( "now +$duration days" ) );
        } else {
            $object->job_expires_at = WPJB_MAX_DATE;
        }
        ...
        foreach($varList as $k => $val) {
            ...
            if( $form_scheme && isset( $form_scheme["field"][$k] ) ) {
                $object->meta->$k->meta_value = serialize($form_scheme["field"][$k]);
            }
    ...
    public function save($append = array())
    {
        ...
        if( $this->value( "custom_expiration_date" ) ) {
            $job_expires_at = date( "Y-m-d H:i:s", strtotime( $this->value( "custom_expiration_date" ) ) );
        } elseif($duration == 0) {
        ...
        if($this->_custom) {
            Wpjb_Model_MetaValue::import("job", "form_code", $this->_custom, $this->getId());
        }

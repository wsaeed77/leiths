...
class Wpjb_Form_AdvancedSearch extends Daq_Form_Abstract
{
    ...
    public function __construct( $id = null, $options = array() )
    {
        if( isset( $options['custom'] ) ) {
            if( $options['custom'] == "wpjb_form_job-search") {
                $this->_custom = "wpjb_form_job_search";
            } elseif( strpos( $options['custom'], "wpjb_form_job-search" ) !== 0 ) {
                $this->_custom = "wpjb_form_job_search_" . $options['custom'];
            } else {
            ...
            if( isset( $forms_list['job_search'] ) ) {
                foreach( $forms_list['job_search'] as $form_code ) {
                    ...
                    if( /*$form_detail['config']['form_is_default'] && */ $form_detail['config']['form_is_active'] ) {
                        $this->_custom = "wpjb_form_job_search_" . $form_detail['config']['form_code'];
                        break;
                    }
                } 
            }
    ...
    public function init()
    {
        ...
        if(!$wp_rewrite->using_permalinks()) {
            ...
            $e->setValue($instance->env("mode")==1 ? wpjb_conf("link_jobs") : get_the_ID());
            $this->addElement($e, "_internal");
        }

...
class Wpjb_Form_Abstract_Resume extends Daq_Form_ObjectAbstract
{
    ...
    public function __construct( $id = null, $options = array() )
    {
        ...
        if( isset( $resume->meta->form_code ) && $resume->meta->form_code->value() != null ) {
            $this->_custom = $resume->meta->form_code->value();
            ...
            if( $options['custom'] == "wpjb_form_resume") {
                $this->_custom = "wpjb_form_resume";
            } elseif( strpos( $options['custom'], "wpjb_form_resume" ) !== 0 ) {
                $this->_custom = "wpjb_form_resume_" . $options['custom'];
            } else {
            ...
            if( isset( $forms_list['resume'] ) ) {
                foreach( $forms_list['resume'] as $form_code ) {

                    $form_detail = maybe_unserialize( get_option( $form_code ) );

                    if( /*$form_detail['config']['form_is_default'] && */ isset( $form_detail['config'] ) && $form_detail['config']['form_is_active'] ) {
                        $this->_custom = "wpjb_form_resume_" . $form_detail['config']['form_code'];
                        break;
                    }
                } 
            }
    ...
    public function save($append = array())
    {
        ...
        Wpjb_Model_ResumeSearch::createFrom($this->getObject());

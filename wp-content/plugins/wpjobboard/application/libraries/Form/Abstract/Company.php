...
abstract class Wpjb_Form_Abstract_Company extends Daq_Form_ObjectAbstract 
{
    ...
    public function __construct( $id = null, $options = array() )
    {
        ...
        if( isset( $company->meta->form_code ) && $company->meta->form_code->value() != null ) {
            $this->_custom = $company->meta->form_code->value();
            ...
            if( $options['custom'] == "wpjb_form_company") {
                $this->_custom = "wpjb_form_company";
            } elseif( strpos( $options['custom'], "wpjb_form_company" ) !== 0 ) {
                $this->_custom = "wpjb_form_company_" . $options['custom'];
            } else {
            ...
            if( isset( $forms_list['company'] ) ) {
                foreach( $forms_list['company'] as $form_code ) {
                    ...
                    if( /*$form_detail['config']['form_is_default'] && */ $form_detail['config']['form_is_active'] ) {
                        $this->_custom = "wpjb_form_company_" . $form_detail['config']['form_code'];
                        break;
                    }
                } 
            }
    ...
    public function save($append = array())
    {
        if($this->isNew() && is_admin() && $this->value("_user_id")) {
            ...
            $user->add_role( apply_filters( "wpjb_user_role", "employer", "employer" ) );

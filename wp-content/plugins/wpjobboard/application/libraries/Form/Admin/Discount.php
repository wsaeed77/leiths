...
class Wpjb_Form_Admin_Discount extends Daq_Form_ObjectAbstract
{
    ...
    public function save($append = array()) 
    {
        ...
        if( $stripe->conf("secret_key") && in_array( $request->post( "discount_for" ), array( Wpjb_Model_Pricing::PRICE_EMPLOYER_MEMBERSHIP, Wpjb_Model_Pricing::PRICE_CANDIDATE_MEMBERSHIP) ) ) {
            ...
            $coupon_id = $request->post( "code" );
            try {
                $coupon_exists = \Stripe\Coupon::Retrieve( $coupon_id );
            } catch( Exception $e ) {
                $coupon_exists = false;
            }
            ...
            if( !$coupon_exists ) {
                ...
                if( $request->post( "type" ) == 1 ) {
                    $params['percent_off'] = $discount_amount;
                } else {
                    $params['amount_off'] = $discount_amount;
                    $params['currency'] = $request->post("currency");
                }

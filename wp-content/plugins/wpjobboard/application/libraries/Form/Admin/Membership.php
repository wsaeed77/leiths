...
class Wpjb_Form_Admin_Membership extends Daq_Form_ObjectAbstract
{
    ...
    public function init() 
    {
        ...
        $mlist = maybe_unserialize($this->getObject()->package);
        ...
        if( $package->price_for == Wpjb_Model_Pricing::PRICE_EMPLOYER_MEMBERSHIP) {
            ...
            $pricing = new Wpjb_Model_Pricing( $this->getObject()->package_id );

            $e = $this->create("have_access", "select");
            if( isset( $pricing->meta->have_access ) ) {
                $e->setValue( explode( ",", $pricing->meta->have_access->value() ) );
            }
    ...
    public function save($append = array())
    {
        ...
        if( $package->price_for == Wpjb_Model_Pricing::PRICE_EMPLOYER_MEMBERSHIP) {
            ...
            if( Daq_Request::getInstance()->get( "action" ) == "add" ) {

                if( isset( $package->meta->package ) ) {
                    $data = maybe_unserialize( $package->meta->package->value() );
                }
            ...
            $q = new Daq_Db_Query();
            $resume_arr = $q->select()->from("Wpjb_Model_Resume t")->where("t.user_id = ?", $this->getObject()->user_id)->execute();
            if( isset( $resume_arr[0]) && !empty( $resume_arr[0] ) ) {
                $resume = $resume_arr[0];
                $resume->featured_level = $data['featured_level'];
                $resume->save();
            }
        }

...
class Wpjb_Form_Admin_Config_Stripe extends Wpjb_Form_Abstract_Payment 
{
    public function init()
    {
        ...
        $e = $this->create("stripe_require_address", "checkbox");
        $e->setBoolean(true);
        $e->setLabel(__("Require Address", "wpjobboard"));
        $e->setValue($this->conf("stripe_require_address"));
        $e->addOption("1", "1", __("User needs to fill his address when making payment.", "wpjobboard"));
        $this->addElement($e, "stripe");

        $e = $this->create("_webhook_url");
        $e->setValue(admin_url( 'admin-ajax.php?action=wpjb_stripe_main'));
        $e->setLabel(__("WebHook Endpoint", "wpjobboard"));

...
class Wpjb_Form_Admin_Config_Google extends Daq_Form_Abstract
{
    ...
    public function init()
    {
        ...
        $e->setLabel(__("Google API Key (Public)", "wpjobboard"));
        $e->addFilter(new Daq_Filter_Trim);
        $this->addElement($e, "default");
    
        $e = $this->create("google_api_key_private");
        $e->setValue($instance->getConfig("google_api_key_private"));
        $e->setLabel(__("Google API Key (Private)", "wpjobboard"));
        $e->addFilter(new Daq_Filter_Trim);
        $this->addElement($e, "default");

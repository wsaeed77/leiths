...
class Wpjb_Form_Admin_Email extends Daq_Form_ObjectAbstract
{
    ...
    public function init()
    {
        ...
        $objects = $evars->getEmailObjects( $this->getObject() );
        ...
        if( isset( $objects ) ) {
            foreach( $objects as $object_name ) {
                ...
                $values = json_decode( $this->_object->files, true );      
                $field_name = "mail_file_" . $object_name;
                
                if( !$values && ( $this->getObject()->name == "notify_employer_new_application" || $this->getObject()->name == "notify_admin_new_application" ) ) {
                    $values["mail_file_application"][] = "file";
                }
                
                $e = $this->create( $field_name, "select" );
                $e->setLabel( sprintf( __( "Attachment (%s) ", "wpjobboard"), $object_name ) );
                $e->setHint( sprintf( __( "Files added to selected fields will be attached to this e-mail.", "wpjobboard"), $object_name ) );
                $e->setMaxChoices( 999 );
                if( isset( $values[$field_name] ) ) {
                    $e->setValue( $values[$field_name] );
                }
                
                if( $object_name == "job" ) {
                    $form = new Wpjb_Form_AddJob();
                } elseif( $object_name == "company" ) {
                    $form = new Wpjb_Form_Frontend_Company();
                } elseif( $object_name == "resume" ) {
                    $form = new Wpjb_Form_Resume();
                } elseif( $object_name == "application" ) {
                    $form = new Wpjb_Form_Apply();
                } elseif( $object_name == "contact_form" ) {
                    $form = new Wpjb_Form_Resumes_Contact();
                } else{
                    continue;
                }
                ...
                foreach( $form->getFields() as $field ) {
                    if( $field->getType() == "file" ) {
                        $e->addOption( $field->getName(), $field->getName(), $field->getLabel() );
                    }
                }
    ...
    public function save($append = array()) 
    {
        ...
        foreach( $objects as $object_name ) {
            ...
            $field_name = "mail_file_" . $object_name;
            $files[$field_name] = $this->value( $field_name );
        }
 
        
        $append["files"] = json_encode( $files );

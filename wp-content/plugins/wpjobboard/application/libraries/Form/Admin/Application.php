...
class Wpjb_Form_Admin_Application extends Wpjb_Form_Abstract_Application
{
    ...
    public function init()
    {
        ...
        $e = $this->create("rating", "text");
        $e->setLabel( "Rating" );
        $e->addFilter(new Daq_Filter_Int());
        if( isset( $this->_object->meta->rating ) && $this->_object->meta->rating->value() ) {
            $e->setValue( $this->_object->meta->rating->value() );
        } else {
            $e->setValue( 0 );
        }
    ...
    public function save($append = array())
    {
        ...
        Wpjb_Model_MetaValue::import( "apply", "form_code", $form_code, $this->getId() );

...
class Wpjb_Model_ResumeSearch extends Daq_Db_OrmAbstract
{
    ...
    public static function search($params)
    {
        ...
        /**
         * @var $candidate_zip_code string
         */
        $candidate_zip_code = null;

        /**
         * @var $candidate_country int
         */
        $candidate_country = null;

        /**
         * @var $candidate_state string
         */
        $candidate_state = null;

        /**
         * @var $candidate_location string
         */
        ...
        if( !current_user_can('administrator') && isset( $is_searchable[0] ) && $is_searchable[0] == 1) {
        ...
        if($headline) {
            $select->where("headline LIKE ?", "%$headline%");
        }

        if($candidate_country) {
            $select->where("candidate_country = ?", $candidate_country);
        }

        if($candidate_state) {
            $select->where("candidate_state LIKE ?", "%$candidate_state%");
        }

        if($candidate_zip_code) {
            $select->where("candidate_zip_code = ?", $candidate_zip_code);
        }
        
        if($candidate_location) {
            $select->where("candidate_location LIKE ?", "%$candidate_location%");
        }
        ...
        if($t>0) {
            if($t == 1) {
                ...
                $select->where($q, "+".  str_replace(" ", " +", (string)$query));
            } else {

...
class Wpjb_Model_Company extends Daq_Db_OrmAbstract
{
    ...
    public function getLogoUrl($resize = null)
    {
        ...
        if(!isset($file[0])) {
            ...
            if(isset($link[0]["url"])) {
                return apply_filters( "wpjb_company_logo_url", $link[0]["url"], $this, $resize, "link");
            } else {
                return apply_filters( "wpjb_company_logo_url", null, $this, $resize, "none");
            }
        }
        ...
        return apply_filters( "wpjb_company_logo_url", $upload["baseurl"]."/".$filename, $this, $resize, "url");
    }
    ...
    public function delete($delete = self::DELETE_FULL)
    {
        ...
        } elseif( $user->exists() && $delete == self::DELETE_PARTIAL ) {
    ...
    public static function import($item) 
    {
        ...
        $have_user = get_user_by( "email", $item->user_email );
        $q = new Daq_Db_Query(); 
        $have_company = $q->select()->from("Wpjb_Model_Company t")->where( "t.user_id = ?", $have_user->ID )->fetchColumn();
        
        if(isset($item->id)) {
            $id = (int)$item->id;
        } elseif( isset( $have_company ) ) {
            $id = $have_company;
        } else {
            $id = null;
        }
        ...
        $object->company_info = wpjb_remove_emojis( (string)$item->company_info );
        $object->company_country = wpjb_resolve_country( $item->company_country );
        ...
        if($roles[0] == "subscriber") {
            wp_update_user(array(
                "ID" => $user_id,
                "role" => apply_filters( "wpjb_user_role", "employer", "employer" )
            ));
        }
        ...
        $result = apply_filters("wpjb_imported_company_output", $result, $object->id, $exists, $item );
    ...
    public function export(Daq_Helper_Xml $xml = null) 
    {
        ...
        $xml->tagCIf("company_info", $this->company_info);
    ...
    public function cpt() 
    {
        ...
        if(!$this->post_id || !get_post($this->post_id)) {
            ...
            $post_id = wp_insert_post(array(
                ...
                "post_author" => absint($this->user_id)
            ));

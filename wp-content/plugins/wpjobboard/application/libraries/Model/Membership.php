...
class Wpjb_Model_Membership extends Daq_Db_OrmAbstract 
{
    ...
    public function save()
    {
        ...
        foreach($this->_field as $key => $value) {
            ...
            switch($value["type"]) {
                ...
                case "textarea": $value["value"] =  strval($value["value"] ); break;
            }
    ...
    public static function getPackageSummary($package_id, $user_id)
    {
        ...
        $result->started_at = $member[0]->started_at;
            ...
            foreach($package as $group => $product) {
                ...
                foreach($product as $id => $use) {
                    ...
                    if( $result->bundle[$group][$id]["status"] == "unlimited" ) {
                        ...
                        $result->bundle[$group][$id]["usage"]+= intval($use["usage"]);
                    }
                    
                    $result->bundle[$group][$id]["used"] += intval($use["used"]);
                }
            }

...
class Wpjb_Model_Resume extends Daq_Db_OrmAbstract
{
    ...
    public function getAvatarUrl($resize = null)
    {
        ...
        if(!isset($file[0])) {
            ...
            if(isset($link[0]["url"])) {
                return apply_filters( "wpjb_resume_logo_url", $link[0]["url"], $this, $resize, "link");
            } else {
                return apply_filters( "wpjb_resume_logo_url", null, $this, $resize, "none");
            }
        }
        ...
        return apply_filters( "wpjb_resume_logo_url", $upload["baseurl"]."/".$filename, $this, $resize, "url");
    }
    ...
    public static function import($item) 
    {
        ...
        $q = new Daq_Db_Query(); 
        $have_resume = $q->select()->from("Wpjb_Model_Resume t")->where( "t.user_id = ?", $have_user->ID )->fetchColumn();
        ...
        $object->description = wpjb_remove_emojis( (string)$item->description );
        $object->created_at = (string)$item->created_at;
        $object->modified_at = (string)$item->modified_at;
        $object->candidate_country = wpjb_resolve_country( $item->candidate_country );
        ...
        $result = apply_filters("wpjb_imported_resume_output", $result, $object->id, $exists, $item );
    ...
    {
        $form_args = apply_filters( "wpjb_form_scheme", array( "custom"=>$this->meta->form_code->value() ), "resume", $this->id );
    ...
    public function getMeta($param = array()) {
        ...
        $form_scheme = unserialize( $form_scheme_s );

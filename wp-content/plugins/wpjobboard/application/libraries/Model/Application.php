...
class Wpjb_Model_Application extends Daq_Db_OrmAbstract
{
    ...
    protected function _load( $id ) {
        ...
        $this->_copy = array( 'id' => $id, 'status' => $this->status );
    }
    ...
    public static function import($item) 
    {
        ...
        if(isset($item->candidate_id)) {
            $c = new Wpjb_Model_Resume((int)$item->candidate_id);
    ...
    public function csv($columns) {
        ...
        foreach($columns as $column) {
            ...
            if($o[$object]->get($property)) {
                ...
                if( $object == "application" && $property == "status") {
                    $status = wpjb_get_application_status( $o[$object]->get($property) );
                    $data[$column] = $status["label"];
                } else {
    ...
    public static function search($params = array())
    {
        ...
        } else if( ( $status || $status === 0 ) && is_numeric($status)) {
        ...
        if($applicant_name) {
            $select->where("applicant_name LIKE ?", "%$applicant_name%");
        }
    ...
    public function toArray() {
        ...
        $arr["admin_url"] = wpjb_admin_url("application", "edit", $this->id);

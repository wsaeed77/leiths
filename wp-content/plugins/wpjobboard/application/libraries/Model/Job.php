...
{    
    ...
    public function getTag() {
        return apply_filters( "wpjb_get_tags", parent::getTag() );
    }
    ...
    public function getLogoUrl($resize = null)
    {
        ...
        if(!isset($file[0])) {
            ...
            if(isset($link[0]["url"])) {
                return apply_filters( "wpjb_job_logo_url", $link[0]["url"], $this, $resize, "link");
            } else {
                return apply_filters( "wpjb_job_logo_url", null, $this, $resize, "none");
            }
        }
        ...
        return apply_filters( "wpjb_job_logo_url", $upload["baseurl"]."/".$filename, $this, "url");
    }
    ...
    public function locationToString()
    {
        ...
        if(strlen(trim($this->job_address))>0) {
            $arr[] = $this->job_address;
        }
    ...
    public function delete( $params = array() )
    {
        ...
        if( !isset( $params["remove_payment_history"] ) || $params["remove_payment_history"] == "delete" ) {
            foreach($object as $ph) {
                $ph->delete();
            }
        } else {
            foreach($object as $ph) {
                $payment_history = new Wpjb_Model_Payment( $ph->id );
                $payment_history->object_id = 0;
        ...
        if( !isset( $params["remove_apps"] ) || $params["remove_apps"] == "full" ) {
            foreach($object as $app) {
                $app->delete();
            }
        } else {
            foreach($object as $app) {
                $application = new Wpjb_Model_Application( $app->id );
                $application->job_id = 0;
                $application->save();
            }
        }
    ...
    public function location()
    {
        ...
        // Remove Empty
		array_filter( $addr, function($value) { return !is_null( $value ) && $value !== ''; } );
    ...
    public function export(Daq_Helper_Xml $xml = null) 
    {
        ...
        $xml->tagIf("job_address", $this->job_address);
        ...
        foreach($this->file as $category => $files) {
            ...
            foreach($files as $file) {
                ...
                $xml->open("file");
                $xml->tag("path", str_replace(array("_","&"), array("-", "-"), $filename));
    ...
    public static function import_new($item) 
    {
        ...
        $job->job_address = (string)$item->job_address;
        ...
        do_action("wpjb_job_saved", $job);
        
        if($job->is_approved && (!$exists || !$approved)) {
            do_action("wpjb_job_published", $job);
        }
    ...
    public static function import($item) 
    {
        ...
        $default->job_created_at = date("Y-m-d");
        $default->job_modified_at = date("Y-m-d");
        $default->job_expires_at = date("Y-m-d", strtotime("today +" . wpjb_conf("default_job_duration", 30) . " day"));
        $default->is_approved = 1;
        $default->is_active = 1;
        $default->is_filled = 0;
        $default->is_featured = 0;
        ...
        $job->job_title = (string)$item->job_title;
        $job->job_slug = (string)$item->job_slug;
        $job->job_description = wpjb_remove_emojis( (string)$item->job_description );
        $job->job_created_at = (string)$item->job_created_at;
        $job->job_modified_at = (string)$item->job_modified_at;
        $job->job_expires_at = (string)$item->job_expires_at;
        $job->job_country = wpjb_resolve_country( $item->job_country );
        $job->job_state = (string)$item->job_state;
        $job->job_zip_code = (string)$item->job_zip_code;
        $job->job_city = (string)$item->job_city;
        $job->job_address = (string)$item->job_address;
        ...
        $result = apply_filters("wpjb_imported_job_output", $result, $job->id, $exists, $item );
    ...
    public static function importCsv($data) 
    {
        ...
        foreach($data as $temp => $value) {
            ...
            if(in_array($k, $fields)) {
                ...
                if( !is_serialized( $value ) && stripos($value, ";")) {
                    $value = explode(";", $value);
                } elseif(empty($value)) {
                    $value = array();
                } else {
                ...
                $import = apply_filters( "wpjb_job_csv_import_column_unknown", $import, $temp, $value );
            }
        }
        //print_r($import);exit;
    ...
    protected static function _resolve($tag) 
    {
        ...
        $query->select();
        $query->from("Wpjb_Model_Tag t");
        $query->where("type = ?", (string)$tag->type);
        $query->where("slug = ?", (string)$tag->slug);
        $query->limit(1);
        ...
        if(empty($result)) {
            ...
            $t->type = (string)$tag->type;
            $t->slug = (string)$tag->slug;
            $t->title = (string)$tag->title;
    ...
    public function cpt() 
    {
        ...
        if($this->employer_id) {
            $company = new Wpjb_Model_Company($this->employer_id);
            $post_author = $company->user_id;
        } else {
        ...
        if(!$this->post_id || !get_post($this->post_id)) {
            ...
            $post_id = wp_insert_post(array(
                ...
                "post_content" => (string)$this->job_description,
    ...
    public function getMetaRowsData() {
        ...
        $showExp = 0;
        $expires = null;
        if( wpjb_conf( "show_expiration_date", false ) !== false ) {
            $showExp = 1;
            $expires = wpjb_date_display(get_option('date_format'), $job->job_expires_at);
        }
    ...
    public function getMeta($param = array()) {
        $list = parent::getMeta($param);

        if(! $this->exists() || !$this->meta->form_code) {
            return $list;
        }

        $form_scheme_s = get_option( $this->meta->form_code->value() );

        if( $form_scheme_s === false ) {
            return $list;
        }

        $form_scheme = maybe_unserialize( $form_scheme_s );

        if( ! is_array( $form_scheme) ) {
            return $list;
        }

        foreach($list as $k => $meta) {
            if( isset( $form_scheme["field"][$k] ) ) {
                $list->$k->meta_value = serialize($form_scheme["field"][$k]);
            }
        }

...
class Wpjb_Model_JobSearch extends Daq_Db_OrmAbstract
{
    ...
    public static function search($params = array())
    {
        ...
        if( isset( $country ) && $country ) {
            $select = self::solve_array( $select, "job_country", $country );
        } elseif( isset( $job_country ) && $job_country ) {
            $select = self::solve_array( $select, "job_country", $job_country );
        }
        
        if( isset( $state ) && $state ) {
            $select = self::solve_array( $select, "job_state", $state );
        } elseif( isset( $job_state ) && $job_state ) {
            $select = self::solve_array( $select, "job_state", $job_state );
        }

        if( isset( $zip_code ) && $zip_code ) {
            $select = self::solve_array( $select, "job_zip_code", $zip_code );
        } elseif( isset( $job_zip_code ) && $job_zip_code ) {
            $select = self::solve_array( $select, "job_zip_code", $job_zip_code );
        }

        if( isset($city) && $city ) {
            $select = self::solve_array( $select, "job_city", $city );
        } elseif( isset( $job_city ) && $job_city ) {
            $select = self::solve_array( $select, "job_city", $job_city );
        }

        if( isset($address) && $address ) {
            $select = self::solve_array( $select, "job_address", $address );
        } elseif( isset( $job_address ) && $job_address ) {
            $select = self::solve_array( $select, "job_address", $job_address );
        }
        ...
        if(isset($id__not_in) && $id__not_in) {
            ...
            $id__not_in = array_map( "trim", $id__not_in );
            $id__not_in = array_map( "intval", $id__not_in );
            $select->where( sprintf( "t1.id NOT IN(%s)", implode( ",", $id__not_in ) ) );
        }
        ...
        if(!empty($category)) {
            ...
            //$groupResults = true;
            //AND t1.id in (select object_id from wp_wpjb_tagged where tag_id IN('3', '4') and object='job')
            $select->where( "t1.id IN (SELECT object_id FROM " . $wpdb->prefix . "wpjb_tagged WHERE tag_id IN (".$category.") AND object='job')" );
        }
        if(!empty($type)) {
            ...
            //$groupResults = true;
            $select->where( "t1.id IN (SELECT object_id FROM " . $wpdb->prefix . "wpjb_tagged WHERE tag_id IN (".$type.") AND object='job')" );
        }
        ...
        if($job_title) {
            $select->where("job_title LIKE ?", "%$job_title%");
        }
        ...
        if($radius && $location) {
            ...
            foreach($locations as $l) {
                $l = trim($l);
                $l = rtrim($l, ',');
                //$select->where( 't1.id IN (SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE t4.location LIKE "%'.$l.'%")');
                $select->where( 't1.id IN (SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE t4.location LIKE ?)', "%".$l."%");
        ...
        if( $posted && intval( $posted ) > 0 ) {
            $select->where( 'DATE_ADD( job_created_at, INTERVAL ' . intval( $posted ) . ' DAY) > NOW()' );
        }
        ...
        if( $query && mb_strlen( $query ) <= apply_filters( "wpjb_fulltext_min_chars", 3 ) && apply_filters( "wpjb_use_default_query_search", true ) ) {
            $select->where( 't1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE t4.title LIKE "%'.$query.'%" OR t4.description LIKE "%'.$query.'%" OR t4.company LIKE "%'.$query.'%" OR t4.location LIKE "%'.$query.'%" )');
            
            $itemsFound = self::_found($select, $groupResults);
        } elseif( $query && mb_strlen( $query ) > apply_filters( "wpjb_fulltext_min_chars", 3 )  && apply_filters( "wpjb_use_default_query_search", true ) ) {
            foreach(array(1, 2, 3) as $t) {
                
                $test = clone $select;
                if($t == 1) {
                    $test->where( 't1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$fulltext.')');
                    //$test->where($fulltext);
                    ...
                    $test->where('t1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$q.')', $modifiers);
                } else {
                    //$test->where($q, $query);
                    $test->where('t1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$q.')', $query);
                }
        ...
        if($t>0 && is_string($query)) {
            if($t == 1) {
                //$select->where($fulltext);
                $select->where( 't1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$fulltext.')');
            } elseif($t == 2) {
                $modifiers = "+".  str_replace(" ", " +", $query);
                $modifiers = str_replace(array("+-", "++"), array("+", "+"), $query);
                //$select->where($q, $modifiers);
                $select->where('t1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$q.')', $modifiers);
            } else {
                //$select->where($q, $query);
                $select->where('t1.id IN(SELECT job_id FROM '.$wpdb->prefix.'wpjb_job_search AS t4 WHERE '.$q.')', $query);
            }
        }
        ...
        $response->page = $page;
        $response->perPage = absint($count);
        $response->count = count($jobList);
        $response->total = $itemsFound;
        $response->sql = $select->toString();
    ...
    public static function solve_array( $select, $field, $value ) {

        if( is_array( $value ) ) {
            $select->where("t1.".$field." IN (?)", (array)$value );
        } else {
            $select->where("t1.".$field." = ?", $value);
        }

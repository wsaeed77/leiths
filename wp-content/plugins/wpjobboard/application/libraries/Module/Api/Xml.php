...
class Wpjb_Module_Api_Xml extends Daq_Controller_Abstract
{
    ...
    public function indeedAction()
    {
        ...
        foreach($jobs->job as $id) {
            ...
            $desc = html_entity_decode( strip_tags( $job->job_description, "<b><h1><h2><h3><h4><h5><h6><br><p><ul><li><strong><em><th><td><tr><table>" ) );
            ...
            if( isset($job->getTag()->type) && isset($job->getTag()->type[0])) {
                $jobType = $job->getTag()->type[0]->title;
            }

            if( isset($job->getTag()->category) && isset($job->getTag()->category[0])) {
                $jobCategory = $job->getTag()->category[0]->title;
            }
            ...
            $this->_tagC("requisitionid", $job->id);
            $this->_tagC("url", wpjb_link_to("job", $job));
            $this->_tagC("company", $job->company_name);
            $this->_tagC("city",  $job->job_city);
            $this->_tagC("postalcode",  $job->job_zip_code);
            $this->_tagC("address",  $job->job_address);
            $this->_tagC("streetaddress",  $job->locationToString());
            $this->_tagC("state",  $job->job_state);
            $this->_tagC("country",  $ct['iso2']);
            $this->_tagC("email", $job->company_email ) ;
            $this->_tagC("description", $desc ) ;
            $this->_tagC("type", $jobType);
            $this->_tagC("category", $jobCategory);
    ...
    public function trovitAction()
    {
        ...
        foreach($jobs->job as $id) {
            ...
            $this->_tagCIf("address",  $job->job_address);
    ...
    public function simplyhiredAction()
    {
        ...
        foreach($jobs->job as $id) {
            ...
            $this->_tagIf("address", $job->job_address);
    ...
    public function rssAction()
    {
        ...
        foreach($result as $id) {
            ...
            foreach($job->tag as $key => $tags) {
                foreach($tags as $tag) {
                    $category = apply_filters("wpjb_xml_api_rss_tag_value", $tag->title, $tag, $key);
                    $item->appendChild($rss->createElement("category", htmlspecialchars( $category ) ) );
                }
            }

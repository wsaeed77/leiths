...
class Wpjb_Module_Admin_Employers extends Wpjb_Controller_Admin
{
    ...
    public function indexAction()
    {
        ...
        if(is_array($forms) && isset($forms["company"])) {
            $forms = $forms['company'];
        } else {
            $forms = array();
        }
        $default_form = wpjb_get_default_form( "company" );
    ...
    public function addAction($param = array())
    {
        ...
        if($this->isPost()) {
            ...
            if( ! $nonce ) {
                ...
                if($isValid) {
                    $this->_addInfo(__("Employer has been created.", "wpjobboard"));
                    $id = $form->save();
                    if(!$id) {
                        $id = $form->getId();
                    }
        ...
        $this->redirectIf($id, sprintf(str_replace("%25d", "%d", wpjb_admin_url("employers", "edit", "%d")), $id));

...
class Wpjb_Module_Admin_Import extends Wpjb_Controller_Admin
{
    ...
    {
        wp_enqueue_script( "wpjb-admin-import" );
    ...
    {
        if( !isset($_REQUEST["_nonce"]) || !wp_verify_nonce( $_REQUEST["_nonce"], "wpjb-xmlupload-action") ) {
            http_response_code(403);
            die('{"jsonrpc" : "2.0", "error" : {"code": 100, "message": "Invalid nonce."}, "id" : "id"}');
        }
        if( ! apply_filters( "wpjb_import_upload_enabled", true ) ) {
            http_response_code(400);
            die('{"jsonrpc" : "2.0", "error" : {"code": 100, "message": "Importing functionality is disabled."}, "id" : "id"}');
        }
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            $allowed_extensions = apply_filters( "wpjb_import_upload_allowed_extensions", array( "zip", "xml", "csv" ) );
            ...
            if( ! in_array( $ext, $allowed_extensions ) ) {
                ...
                http_response_code(400);
                die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "'.$ext.'Trying to upload a file with unallowed extension."}, "id" : "id"}');
            }
    ...
    protected function _handle($file) 
    {
        ...
        if( ! in_array( $ext, $allowed_extensions ) ) {
            unlink($file);
            http_response_code(400);
            die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Trying to upload a file with unallowed extension."}, "id" : "id"}');
        }
        ...
        if($ext == "zip") {
            ...
            if ($zip->open($file) === TRUE) {
                ...
                for($i = 0; $i < $zip->numFiles; $i++) {   
                    ...
                    if(!isset($pathinfo["extension"]) || !in_array($pathinfo["extension"], array("xml", "csv"))) {
                        unlink($file);
                        http_response_code(400);
                        die('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Zip file contains files other than XML."}, "id" : "id"}');            
                    }
                }
    ...
    public function xmlimportAction()
    {
        ...
        $xml = simplexml_load_file($file, 'SimpleXMLElement', LIBXML_PARSEHUGE);
    ...
    public function csvAction()
    {
        ...
        $this->view->nonce = wp_create_nonce("wpjb-xmlupload-action");
    }

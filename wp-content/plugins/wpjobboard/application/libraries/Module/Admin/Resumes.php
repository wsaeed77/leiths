...
class Wpjb_Module_Admin_Resumes extends Wpjb_Controller_Admin
{
    ...
    {        
        ...
        if(isset($forms['resume'])) {
            $forms = $forms['resume'];
        } else {
            $forms = null;
        }
        $default_form = wpjb_get_default_form( "resume" );
        ...
        if($this->isPost()) {
            ...
            if( ! $nonce ) {
                ...
                if($isValid) {
                    ...
                    $form->save();
                    $id = $form->getId();
                    $form = new $form($id); // Reload form for case where form was changed
                } else {
            ...
            if( ! $nonce ) {
                ...
                foreach($this->_request->post("users", array()) as $id) {
                    ...
                    if($delete == Wpjb_Model_Resume::DELETE_FULL) {
                        ...
                        foreach($list as $app) {
                            if($this->_request->post("applications_option") == "delete") {
                                $app->delete();
                            } else {
                                $app->user_id = 0;
                                $app->save();

...
class Wpjb_Module_Admin_Memberships extends Wpjb_Controller_Admin
{
    ...
    protected function _multiDelete($id)
    {
        ...
        try {
            $model = new Wpjb_Model_Membership($id);

            $stripe = new Wpjb_Payment_Stripe();
            $sub_id = Wpjb_Model_MetaValue::getSingle('membership', 'subscription_id', $model->id);
            if( is_object( $sub_id ) && stripos( $sub_id->value, "sub_" ) === 0 ) {
                $stripe->cancelSubsctiption($sub_id->value);
            }

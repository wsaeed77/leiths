...
class Wpjb_Module_Admin_Application extends Wpjb_Controller_Admin
{
    ...
    public function init()
    {
        ...
        $this->_virtual = apply_filters( "wpjb_bulk_actions_functions", array(
           "redirectAction" => array(
               "accept" => array("query", "posted", "job", "filter", "search"),
               "object" => "application"
           ),
           "addAction" => array(
        ...
        foreach(wpjb_get_application_status() as $key => $data) {
            ...
            $success = sprintf( __( "Number of applications marked as %s: {success}", "wpjobboard" ), $data["label"] );
            
            $this->_virtual["_multi"][$data["key"]] = array(
                "callback" => function( $id ) use ($data) {
                    
                    $application = new Wpjb_Model_Application( $id );
                    $application->status = $data['id'];
                    $application->save();
    ...
    public function indexAction()
    {
        ...
        $param = array();
        $query = "";
        $this->view->advanced_filters = false;
        $this->view->rquery = "";

        if($this->_request->get("query")) {
            $query = sanitize_text_field( $this->_request->get("query") );
        
            $this->view->rquery = $this->readableQuery($query);
            $param = $this->deriveParams($query, new Wpjb_Model_Application);
        } else if($this->_request->get("search")) {
            $this->view->advanced_filters = true;
            $param = $this->_request->get("search");
        ...
        if($this->_request->get("sort") && in_array( strtolower( $this->_request->get("sort") ), apply_filters( "wpjb_sort_applications_allowed_fields", array( "__rating", "applied_at" ) ) ) ) {
            add_filter("wpjb_applications_query", array($this, "sortByRating"));
            $navigation["sort"] = $this->_request->get("sort");
        } elseif( apply_filters( "wpjb_sort_applications_allow_unsafe_sort", false ) ) {
            $navigation["sort"] = $sort;
        }

        if( $this->_request->get("order") && in_array( strtolower( $this->_request->get("order") ), array( "asc", "desc" ) ) ) {
        ...
        if(is_array($forms) && isset($forms["apply"])) {
            $forms = $forms['apply'];
        } else {
            $forms = array();
        }
        $default_form = wpjb_get_default_form( "apply" );

...
class Wpjb_Module_Ajax_Main
{
    ...
    public static function cleanupAction()
    {

    }
    
    public static function googleapiAction()
    {
        ...
        $query = http_build_query(array(
            "address" => $address,
            "sensor" => "false",
            "key" => wpjb_conf( "google_api_key_private", wpjb_conf("google_api_key") )
        ));
    ...
    public static function jobsAction() {
        ...
        $jobs = wpjb_find_jobs(array(
            "filter" => "all",
            "query" => Daq_Request::getInstance()->get("term"),
            "count" => 10
        ));
        ...
        foreach($jobs->job as $job) {
            ...
            $label = sprintf("%s (ID :%d)", $job->job_title, $job->id);
            
            $object = new stdClass();
            $object->label = $label;
            $object->value = $job->job_title;
            $object->id = $job->id;
            $object->role = "";
            $object->hint = sprintf(__("ID: %d", "wpjobboard"), $job->id);
            $json[] = $object;
        }
    ...
    public static function hashAction() {

        if(defined("NONCE_KEY")) {
            $nonce = NONCE_KEY;
        } else {
            $nonce = (string)rand(100000, 1000000);
        }

        $response = new stdClass;
        $response->status = 1;
        $response->hash = md5( time() . "-" . home_url() . NONCE_KEY );

...
class Wpjb_Module_Ajax_Export {
    ...
    public static function getExportCap() {
        return apply_filters( "wpjb_admin_export_capability", "export");
    }
    
    public static function downloadAction()
    {
        if(!current_user_can(self::getExportCap())) {
            wp_die(__("You do not have privileges required to run export.", "wpjobboard"));
        }

        $request = Daq_Request::getInstance();
        $name = sanitize_file_name($request->get("name"));
    ...
    {   
        ...
        $request = Daq_Request::getInstance();
        $name = sanitize_file_name($request->post("name"));
        $transient = wpjb_session()->get($name);
    ...
    public static function importnowAction() {
        $response = new stdClass;
        $response->result = 0;
        $response->message = "";

        $request = Daq_Request::getInstance();
        $id = $request->post("import_id");
        $nonce = $request->post("import_nonce");

        if(!wp_verify_nonce($nonce, sprintf("wpjb-import-now-%d", $id))) {
            $response->result = -1;
            $response->message = __( "Could not verify the nonce.", "wpjobboard" );
            echo json_encode($response);
            exit;
        }

        $import = new Wpjb_Model_Import($id);

        if(!$import->exists()) {
            $response->result = -1;
            $response->message = sprintf( __( "Import identified with ID %d does not exist.", "wpjobboard" ), $id );
            echo json_encode($response);
            exit;
        }

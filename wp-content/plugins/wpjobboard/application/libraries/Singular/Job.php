...
class Wpjb_Singular_Job extends Wpjb_Shortcode_Abstract {
    ...
    public function listen() {
        ...
        add_action( "template_redirect", array($this, "onRedirect"));
    ...
    public function onInit() {
        ...
        $apply_form_code = null;
        if( isset( $job->meta->application_form_code ) ) {
            $apply_form_code = $job->meta->application_form_code->value();
        }
        if( empty( $apply_form_code ) ) {
            $apply_form_code = null;
        }   

        // create new job application form if needed
        $this->form = new Wpjb_Form_Apply(null, apply_filters( "wpjb_form_scheme", array( "custom"=>$apply_form_code ), "apply", $job->id ) );
        ...
        do_action( "wpjb_application_process_done", $application, $job );
        ...
        if(get_option('permalink_structure')) {
            $url = trailingslashit( rtrim( wpjb_link_to("job", $job), "/" ) . "/applied/" . $job->id );
        } else {
    ...
    public function onRedirect( $template ) {
        if( ! is_singular( "job" ) || current_user_can( "edit_pages" ) ) {
            // do nothing
            return $template;
        }

        $job = wpjb_get_object_from_post_id(get_the_ID(), "job");
        if( is_object( $job ) && in_array( Wpjb_Model_Job::STATUS_ACTIVE, $job->status() ) ) {
            // do nothing
            return $template;
        }

        // do a redirect or issue a 404 error here
        $redirect_expired = trim( apply_filters( "wpjb_front_redirect_expired", wpjb_conf( "front_expired_redirect" ), $job ) );
        if( $redirect_expired ) {
            wp_redirect( $redirect_expired, 301 );
            exit;
        } 
    ...
    public function notifyEmployer($application, $job) {
        ...
        $mail->assign("resume", Wpjb_Model_Resume::current());
        //$mail->addFiles($files);
        $mail->addHeader("Reply-To", $application->email);
        $mail->setTo($notify);
        $mail->send();
    }
    ...
    public function theContent($content) {
        if( apply_filters( "wpjb_singular_ignore_the_content", false, "job" ) ) {
            return $content;
        }
    ...
    /**
     * Trim script and html tags from single job page title
     * 
     * @param string $title 
     * @return string
     */
    ...
    public function main($post_id) {
        ...
        $this->view->premium_members_only = false;
        ...
        if( $job->is_filled == 3 ) {
            $msg = __("This job post is no longer available.", "wpjobboard");
            $this->addError( $msg );
            return wpjb_flash_get();
        } elseif( $job->is_filled > 1 ) {
            $can_apply = false;
        }
        ...
        $this->view->application_methods = $this->getApplicationMethods($job);
    ...
    public function getApplicationMethods($job) {
        ...
        $buttonEmail = new Daq_Helper_Html("a", array(
            "class" => "wpjb-button wpjb-form-toggle wpjb-form-job-apply",
            "href" => wpjb_link_to("job", $job, array("form"=>"apply")) . "#wpjb-scroll",
            "rel" => "nofollow",
            "data-wpjb-form" => "wpjb-form-job-apply"
        ), __("Apply Now", "wpjobboard") . '<span class="wpjb-glyphs wpjb-icon-down-open">&nbsp;</span>' );

        $buttonUrl = new Daq_Helper_Html("a", array(
            "class" => "wpjb-button",
        ...
        $arr = array(
            "email" => array(
                "is_active" => $job->company_email || $job->employer_id ? 1 : 0,
                "button" => $buttonEmail->render(),
                "callback" => array($this, "applicationForm")
            ),
            "url" => array(
                "is_active" => ($application_url !== null),
                "button" => $buttonUrl->render()
            )
        );

        return apply_filters( "wpjb_singular_job_application_methods", $arr, $job );
    }
    ...
    public function googleForJobs() {
        ...
        if(isset($gconf["is_disabled"]) && $gconf["is_disabled"] == "1" ) {
            return;
        }
    ...
    public function applicationForm($key, $method, $job, $can_apply) {
        echo $this->render("job-board", "single-apply");
    }

...
class Wpjb_Singular_Resume extends Wpjb_Shortcode_Abstract {
    ...
    public function listen() {
        add_action( "init", array($this, "initPreview"), 50);
    ...
    public function initPreview() {
        ...
        if($request->get("post_type") == "resume" && absint($request->get("p") > 0)) {
        ...
        if($id) {
            $resume_r = wpjb_get_object_from_post_id($id, "resume");
            $resume_c = Wpjb_Model_Resume::current();
            
            if($resume_r && $resume_c && $resume_r->id == $resume_c->id) {
    ...
    public function theContent($content) {
        if( apply_filters( "wpjb_singular_ignore_the_content", false, "resume" ) ) {
            return $content;
        }
    ...
    /**
     * Trim script and html tags from single resume page title
     * 
     * @param string $title 
     * @return string
     */
    ...
    protected function handleContact($resume) {
        ...
        if($valid) {
            ...
            do_action( "wpjb_resume_contact_form_sent", $resume );

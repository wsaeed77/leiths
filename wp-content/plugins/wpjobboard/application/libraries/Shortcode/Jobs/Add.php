...
class Wpjb_Shortcode_Jobs_Add extends Wpjb_Shortcode_Abstract {
     ...
     * Shortcode router
     * 
     * Either "permalinks" or "get"
     *
     * @var string
     */
    public $router = "permalinks";
    ...
    private function _canPost()
    {
        ...
        if( wpjb_conf( "posting_allow" ) == 4 && !wpjb_if_premium_employer() ) {
            $this->addError(__("Only premium members can post jobs", "wpjobboard"));
            $this->view->canPost = false;
            $this->view->can_post = false;
            if( $employer === null ) {
                return "../default/form";
    ...
    public function sessionSet($key, $value) {
        $id = "wpjb_session_".str_replace("-", "_", (string)wpjb_transient_id());
    ...
    public function init()
    {
        ...
        if( $this->router === "permalinks") {
            ...
            $urls->add = remove_query_arg( "wpjb-step" );
            $urls->preview = add_query_arg( "wpjb-step", "preview" );
            $urls->save = add_query_arg( "wpjb-step", "save" );
            $urls->reset = add_query_arg( "wpjb-step", "reset" );
        }
    ...
    public function add( $params = array() ) {
        ...
        if( !isset( $params['form_code'] ) ) {
            $params['form_code'] = null;
        }
        $form = new Wpjb_Form_AddJob( null, apply_filters( "wpjb_form_scheme", array( "custom"=>$params['form_code'] ), "job", null ) ) ;
        ...
        if( Daq_Request::getInstance()->isPost() ) {
        ...
        if( isset( $jobArr ) && is_array($jobArr)) {
            $form->isValid($jobArr);
        } else {
    ...
    public function preview( $params )
    {
        ...
        $form = new Wpjb_Form_AddJob( null, apply_filters( "wpjb_form_scheme", array( "custom" => $form_code ), "job", null ) );
        ...
        $this->view->job = $form->buildModel( $form_code );

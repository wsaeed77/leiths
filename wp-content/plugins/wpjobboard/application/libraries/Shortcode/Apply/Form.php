...
class Wpjb_Shortcode_Apply_Form extends Wpjb_Shortcode_Abstract
{
    ...
    public function main($atts = array()) {
        ...
        $form = new Wpjb_Form_Apply(null, apply_filters( "wpjb_form_scheme", array( "custom"=> $params['form_code'] ), "apply", null ) );
        ...
        if($request->post("_wpjb_action")=="apply_general" && $can_apply) {
            ...
            if($form->isValid($request->getAll())) {
                ...
                $user = null;
                if($job->exists() && $job->employer_id) {
                    $user = new WP_User($job->getCompany(true)->user_id);
                }

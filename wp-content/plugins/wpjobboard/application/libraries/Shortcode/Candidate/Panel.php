...
class Wpjb_Shortcode_Candidate_Panel extends Wpjb_Shortcode_Panel_Abstract {
    ...
    public function resume() {
        ...
        //if( !wpjb_if_form_exist( $object->meta->form_code->value(), "resume") ) {
        ...
        $form = new Wpjb_Form_Resume($id, apply_filters( "wpjb_form_scheme", array( "custom"=>null ), "resume", $id ) );
        if($this->getRequest()->isPost()) {
            $isValid = $form->isValid($this->getRequest()->getAll());
            if($isValid) {
        ...
        if($object) {
            $this->view->buttons[] = array(
                "tag" => "a", 
                "href" => wpjr_link_to("resume", $object), 
    ...
    public function applications() 
    {
        ...
        $allow_remove = wpjb_conf( "cv_managment_allow_remove_application", false );
        $allowed_statuses = wpjb_conf( "cv_managment_remove_statuses", array() );
        $allowed_delay = wpjb_conf( "cv_managment_remove_duration", 0 );

        if( $request->isPost() && $allow_remove ) {
            $id = $request->post("remov_app_id", null);
            $nonce = $request->post("remov_app_nonce", null);
            if( wp_verify_nonce( $nonce, "wpjb_remove_application" ) && $id > 0 ) {
                ...
                if( in_array( $application->status, $allowed_statuses ) ) {
                    $diff = ( ( time() - strtotime( $application->applied_at ) ) / ( 60 * 60 ) ); 
                    if( $allowed_delay == -2 || ( $allowed_delay > 0 && $allowed_delay <= $diff ) || ( $allowed_delay == -1 && strtotime( $job->job_expires_at ) > time() ) )  {
                        $application->delete();
                    }
                }
            }
        ...
        $this->view->allow_remove = $allow_remove;
        $this->view->allow_statuses = $allowed_statuses;
        $this->view->allow_delay = $allowed_delay;
    ...
    public function membership() {
        ...
        switch($this->getUserPrivs()) {
            case -1: return false; break;
            case -2: return $this->_loginForm(wpjr_link_to("mymembership")); break;
        }

...
class Wpjb_Shortcode_Employer_Panel extends Wpjb_Shortcode_Panel_Abstract {
    ...
    public function removeApplication() {
        ...
        if($this->getRequest()->post("_wpjb_action") != "delete_application") {
            return;
        }
        
        $request = Daq_Request::getInstance();
        $flash = new Wpjb_Utility_Session();
        $form = new Wpjb_Form_Frontend_DeleteApplication($request->post("application_id"));
        $application = $form->getObject();
        $job = new Wpjb_Model_Job( $application->job_id );

        if($job->employer_id != Wpjb_Model_Company::current()->id) {
            $flash->addError(__("This application is not from your job.", "wpjobboard"));
            return;
        }

        if($form->isValid($request->post())) {
            $flash->addInfo(__("Application has been deleted.", "wpjobboard"));
            $flash->save();
            $application->delete();
            wp_redirect($form->value("redirect_to"));
            exit;
    ...
    public function listen() {
        ...
        add_action( "init", array($this, "removeApplication"));
    }
    ...
    public function afterTitle($lname, $link) {
        if($lname == "job_applications" && Wpjb_Model_Company::current()) {

            $apps = $this->_getNewApplicationsCount();
    ...
    protected function _getNewApplicationsCount() {
        $status = wpjb_get_application_status(1);
    ...
    public function employerEdit() {
        ...
        //if( !wpjb_if_form_exist( $company->meta->form_code->value(), "company") ) {
        //    $this->addError( __( "Your profile is based on not existing form. You may lose some data when you will save it!", "wpjobboard" ) );
        //}
        ...
        $form = new Wpjb_Form_Frontend_Company($id, apply_filters( "wpjb_form_scheme", array( "custom"=>null ), "company", $id ) );
    ...
    public function jobEdit()
    {
        ...
        //if( !wpjb_if_form_exist( $job->meta->form_code->value(), "job") ) {
        //    $this->addError( __( "This job is based on not existing form. You may lose some data when you will save it!", "wpjobboard" ) );
        //}

        $this->view = new stdClass();
        $form = new Wpjb_Form_Frontend_EditJob($job->getId(), null, apply_filters( "wpjb_form_scheme", array( "custom"=>null ), "job_edit", $job->getId() ) );
        if($this->getRequest()->isPost()) {
            $isValid = $form->isValid($this->getRequest()->getAll());
            if($isValid) {
    ...
    public function applications() {
        ...
        if( ( $this->view->job_status || $this->view->job_status === "0" ) && in_array($this->view->job_status, $public_ids)) {
            $filter_status = (int)$this->view->job_status;
    ...
    public function application() {
        ...
        if( $job_status !== null ) {
            $apps_args["status"] = absint( $job_status );
        }
        
        $apps = Wpjb_Model_Application::search($apps_args);
    ...
    /**
     * Display Employer Panel / Application Delete
     * 
     * @see wpjb_employer_panel()
     * @example /employer-panel/application-delete/(int)/
     * 
     * @return string   Shortcode HTML
     */
    public function applicationDelete() {
        ...
        if(!get_current_user_id()) {
            return $this->_loginForm(wpjb_link_to("application_delete", $job));
        }
        ...
        if($job->employer_id != $company->getId()) {
            $this->addError(__("Application is not connected with your job.", "wpjobboard"));
            return $this->flash();
        }
        
        $this->view = new stdClass();
        $this->view->action = "";
        $this->view->submit = __("Delete Application", "wpjobboard");
        ...
        $form = new Wpjb_Form_Frontend_DeleteApplication($application->getId());
        $form->getElement("redirect_to")->setValue(wpjb_link_to("employer_panel"));
        ...
        $this->view->breadcrumbs = array(
            array("title"=>__("Home", "wpjobboard"), "url"=>wpjb_link_to("employer_home"), "glyph"=>"wpjb-icon-home"),
            array("title"=>__("Listings", "wpjobboard"), "url"=>wpjb_link_to("employer_panel"), "glyph"=>$this->glyph()),
            array("title"=>sprintf(__("Delete Application '%s'", "wpjobboard"), $application->applicant_name), "url"=>wpjb_link_to("job_application", $application), "glyph"=>$this->glyph()),
        );
    ...
    public function jobAdd() {
        ...
        $params = array();
        if(is_array($this->_atts) && isset($this->_atts["add__form_code"])) {
            $params["form_code"] = $this->_atts["add__form_code"];
        }

        $params = apply_filters( "wpjb_employer_panel_add_params", $params, $this->_atts );

        $content = Wpjb_Project::getInstance()->shortcode->wpjb_jobs_add->add( $params );
    ...
    public function jobPreview() {
        ...
        $content = Wpjb_Project::getInstance()->shortcode->wpjb_jobs_add->preview( $params );
    ...
    public function jobSave() {
        ...
        $content = Wpjb_Project::getInstance()->shortcode->wpjb_jobs_add->save( $params );

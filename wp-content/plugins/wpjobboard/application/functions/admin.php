...
function daq_form_layout(Daq_Form_Abstract $form, $options = array()) {
    ...
    if(isset($exclude_hidden)) {
        $exclude_hidden = explode(",", $exclude_hidden);
        $exclude_hidden = array_map("trim", $exclude_hidden);
    } else {
        $exclude_hidden = array();
    }
?>

<?php echo $form->renderHidden( $exclude_hidden ) ?>
...
<tr valign="top" class="<?php echo sprintf( "wpjb-form-layout-row--%s", $field->getName() ) ?> <?php if($field->hasErrors()): ?>error<?php endif ?>">
    <th scope="row">
...
function wpjb_admin_url($page, $action = null, $id = null, $param = array()) {
    ...
    $arr = array_merge($arr, _wpjb_reduce_empty_params($param));
...
function _wpjb_reduce_empty_params($arr) {
    $result = array();
    foreach( $arr as $k => $v ) {
        if(is_array($v)) {
            $result[$k] = _wpjb_reduce_empty_params($v);
        } elseif(is_scalar($v) && strlen($v)>0) {
            $result[$k] = $v;
        }
    }
    return $result;
...
function wpjb_get_default_form( $type ) {

    $forms_list = get_option( "wpjb_forms_list" ); 

    if( ! isset( $forms_list[$type] ) ) {
        return "wpjb_form_" . $type;
    }
    
    foreach( $forms_list[$type] as $form_code ) {

        $form_detail = maybe_unserialize( get_option( $form_code ) );

        if( isset( $form_detail['config'] ) && $form_detail['config']['form_is_active'] ) {
            return "wpjb_form_" . $type . "_" . $form_detail['config']['form_code'];
        }
    } 

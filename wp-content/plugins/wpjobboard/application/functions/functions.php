...
function wpjb_recaptcha_form( $field ) {
    ...
    if($captchaType == "re-captcha") {
        ...
        $recaptcha_options = apply_filters(
            "wpjb_recaptcha_field_options",
            array(
                "class" => "g-recaptcha",
                "data-sitekey" => wpjb_conf("front_recaptcha_public"),
                "data-theme" => $field->getAttr( "data-theme" ) ? $field->getAttr( "data-theme" ) : wpjb_conf("front_recaptcha_theme", "light"),
                "data-type" => wpjb_conf("front_recaptcha_media", "image"),
                "data-size" => $field->getAttr( "data-size") ? $field->getAttr( "data-size" ) : null
            ),
            $field
        );

        $key = Wpjb_Project::getInstance()->getConfig("front_recaptcha_public");
        $html = new Daq_Helper_Html("div", $recaptcha_options );
...
function wpjr_can_browse_err() {
    ...
    switch($c) {
        ...
        case "1/5":
            ...
            break;
        case "1/6":
            return __("Resume details visible only for employers with at least one application from this candidate.", "wpjobboard");
            break;
    }
...
/**
 * Check if Employer have access to provided page
 * 
 * @param int $page_id
 * @param int $resume_id
 * @return boolean
 */
function wpjb_employer_have_access( $page_id, $employer_id = null ) {
    
    
    $restricted_pages = wpjb_conf( "employer_members_restricted_pages", array() );
    ...
    $access_config = wpjb_conf("employer_members_have_access");
    ...
    // Is Admin or Candidate
    if( current_user_can("manage_resumes") || current_user_can( 'manage_options' ) ) {
        return true;
    }
    ...
    // Registred candidates 
    if( $access_config == 1 && $employer_id > 0 ) {
        return true;
    } elseif( $access_config == 1 ) {
        return false;
    }
...
function wpjb_resolve_country( $country ) {

    // country is already number
    if( is_numeric( (int)$country ) && (int)$country > 0 ) {
        return (int) $country;
    }

    // country is 2 char string (eg. GB)
    if( is_string( $country ) && strlen( $country ) == 2 ) {
        return (int) Wpjb_List_Country::getByAlpha2( $country )['code'];
    }  

    // country is 3 char string (eg. GBR)
    if( is_string( $country ) && strlen( $country ) == 3 ) {
        return (int) Wpjb_List_Country::getByAlpha3( $country )['code'];
    }  

    // Try to find by name
    foreach( Wpjb_List_Country::getAll() as $country ) {
...
function wpjb_if_premium_employer() {
    ...
    if (!empty($result)) {       
        foreach($result as $pricing) {
            $summary = Wpjb_Model_Membership::getPackageSummary( $pricing->id, $user_id );
...
function wpjb_get_print_capability() {
    return apply_filters( 'wpjb_get_print_capability', 'edit_pages' );
}

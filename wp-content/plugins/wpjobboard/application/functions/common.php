...
function wpjb_price($amount, $currency) {
    ...
    return apply_filters( "wpjb_price", $price, $amount, $currency );
}
...
function wpjb_upload_dir_secure( $d, $object, $field, $id, $index ) {
    global $blog_id;

    $name = sprintf( 'fs_secure_%s', $object );
    $name_exclude = sprintf( 'fs_secure_%s_exclude', $object );

    if( wpjb_conf( $name ) !== "1" ) {
        return $d;
    }

    $exclude_string = wpjb_conf( $name_exclude );
    if( ! empty( $exclude_string ) ) {
        $exclude = array_map( "trim", explode( ",", $exclude_string ) );
    } else {
        $exclude = array();
    }
    ...
    $folder = wpjb_conf( 'fs_folder' );

    if( $folder ) {
        $d["basedir"] = rtrim( $folder, "/" ) . "/" . $d["dir"];
    }

    $url = wpjb_api_url("secure/file");
    ...
    if( wpjb_conf( 'fs_hash_path') == "1" ) {
        ...
        $dir = wpjb_cypher_text($d["dir"], $hash_key);
    } else {
        $dir = $d["dir"];
    }

    $d["baseurl"] = $url . $blog_id . "/" . $dir;
...
function wpjb_transient_id() {
    ...
    $sid = apply_filters( "wordpress_wpjb_transient_id", "wpjb_transient_id" );
    ...
    if(!headers_sent() && (!isset($_COOKIE[$sid]) || empty($_COOKIE[$sid]))) {
        ...
        $cookiepath = apply_filters( "wpjb_transient_id_cookiepath", COOKIEPATH );
...
function wpjb_form_field_upload(Daq_Form_Element $e, $form = null) {
    ...
    $form_code = "wpjb_form_" . $path["object"];
    if( isset( $form ) && $form->hasElement("form_code") ) {
        $form_code = $form->getElement("form_code")->getValue();
    } else {
        ...
        if( isset( $forms_list[$po] ) ) {
            foreach( $forms_list[$po] as $form_code ) {

                $form_detail = maybe_unserialize( get_option( $form_code ) );
                
                if( $form_detail['config']['form_is_active'] ) {
                    //$form_code = "wpjb_form_job_" . $form_detail['config']['form_code'];
                    $form_code = $form_detail['config']['form_code'];
                    break;
                }
            } 
        }
    ...
    $allowed = array( "wpjb-job", "wpjb-application", "wpjb-employers", "wpjb-resumes" );
    ...
    if(Wpjb_Utility_Registry::has("form-upload-init")) {
        ...
        if(is_admin() && in_array( $request->get("page"), $allowed)) {
            add_filter("admin_footer", "wpjb_form_field_upload_footer");
            wp_enqueue_style('wpjb-glyphs');
        } else {
            add_filter("wp_footer", "wpjb_form_field_upload_footer", 1000 );

...
function wpjb_event_expiring_jobs() 
{
    ...
    $days_to_expire = apply_filters( "wpjb_expiring_jobs_date", 5 );
    $dt = date("Y-m-d", strtotime("today +".$days_to_expire." day"));
...
function wpjb_event_subscriptions_daily() {
    ...
    foreach($result as $alert) {
        ...
        global $wpdb;
        $wpdb->query( "SELECT MD5(CONCAT(".$alert->id.", '|', '".$alert->email."')) AS hash");
        $hash = $wpdb->last_result[0]->hash;
        ...
        $mail->assign("unsubscribe_url", wpjb_api_url("action/alert", array(
            "delete" => $hash //md5($alert->id."|".$alert->email)
        )));

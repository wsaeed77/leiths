...
<div class="alignleft actions">
    <select name="action" id="wpjb-action1">
        ...
        <?php do_action( "wpjb_bulk_actions", "jobType" ); ?>
    </select>
...
<table cellspacing="0" class="widefat post fixed wp-list-table">
	...
	<tr valign="top" class="<?php if($i%2==0): ?>alternate <?php endif; ?>  author-self status-publish iedit">
            ...
            <td data-colname="<?php esc_attr_e("Total Jobs", "wpjobboard") ?>" class=""><?php echo isset($stat[$item->id]->jobs_total) ? $stat[$item->id]->jobs_total : "0" ?></td>
            <td data-colname="<?php esc_attr_e("Total Resumes", "wpjobboard") ?>" class=""><?php echo isset($stat[$item->id]->resumes_total) ? $stat[$item->id]->resumes_total : "0" ?></td>
        </tr>
    <?php endforeach; ?>
    </tbody>
</table>

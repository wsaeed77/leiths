jQuery(function($) {
    ...
    $("#ve-save-form").click(function() {
        ...
        jQuery("#ve-save-failed-custom").hide();
        ...
        jQuery.ajax({
            url: ajaxurl,
            data: {
                action: "wpjb_customfields_checkname",
                code: $("#form_code").val(),
            ...
            success: function(res) {
                if( res.code != 200 ) {
                    ...
                    jQuery("#ve-save-failed-custom").html( msg.trim() );
                    jQuery("#ve-save-failed-custom").fadeIn("fast");
    ...
    function wpjb_save_cf_form() {
        ...
        var form_is_active = 0;
        if( $("#form_is_active").prop("checked") ) {
            form_is_active = 1;
        }
        var form_is_default = 0;
        if( $("#form_is_default").prop("checked") ) {
            form_is_default = 1;
        }
    ...
    var is_new = jQuery("#form_code").val();
    jQuery("#form_label").on( "input", function() {
        
        if( is_new == "" ) {
            var code = $(this).val().toLowerCase().replace(/[^A-Z0-9]+/ig, "_");
    ...
    propertiesShow: function(r, object) {
        ...
        jQuery.each(r._item, function(k, v) {
            ...
            if(input.attr("type") == "checkbox") {
                ...
                input.prop( "checked", v ? "checked" : false );
            } else {
                input.val(v);
            }
        });
...
function VisualEditorInputText() {
    ...
    this.propertiesShow = function() {
        ...
        w.find("select[name=validation_rules]").on("change", $self._onToogleValidators);
        ...
        $self._toogleValidators(w.find("select[name=validation_rules]").val());
    },
    
    this._onToogleValidators = function(e) {
        $self._toogleValidators(jQuery(e.currentTarget).val());
    }

    this._toogleValidators = function(rules) {

        var w = jQuery("#TB_window");

        if(rules === "Daq_Validate_Date") {
            w.find(".field-year-range").show();
        } else {
            w.find(".field-year-range").hide();
        }

        if(rules === "Daq_Validate_Url") {
            w.find(".field-target").show();
        } else {
            w.find(".field-target").hide();
        }
